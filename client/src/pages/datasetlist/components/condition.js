...
const Condition = (inputText) => {
  ...
  useEffect(() => {
    ...
    const getGrouplist = async () => {
      try {
        await axios.get( 
          `${process.env.REACT_APP_BACKEND_URI}api/ckan/group_list`,
              {headers:{'Authorization': process.env.REACT_APP_CKAN_TOKEN}}
        )
        .then(response => {
    ...
    const getTaglist = async () => {
      try {
        await axios.get( 
          `${process.env.REACT_APP_BACKEND_URI}api/ckan/tag_list`,
              {headers:{'Authorization': process.env.REACT_APP_CKAN_TOKEN}}
        )
        .then(response => {
  ...
  function getDatasetList(){
    const getDatasetlist = async () =>{
      ...
      try{
        await axios.get(
          `${process.env.REACT_APP_BACKEND_URI}api/ckan/${condition.URL}`,
            {
              params:{
                id:id
              },
    ...
    <div className="package-condition-div-style">
        ...
        <div>
          <div className="condition-list-div">
            <button onClick={orglistStateChange} className="icon-button">
              {!isOrgListVisible &&(<div><KeyboardArrowRightIcon/></div>)}
              {isOrgListVisible &&(<div><KeyboardArrowDownIcon/></div>)}
            </button>
            <font className="package-condition-title">
              Organizations
            </font>
          </div>
          <div>
              ...
              <div>
                {
                  orglist.map((element, index)=>{
                    return(
                      <button
                        className="package-condition-item"
                        value={element}
                        key={index}
                        onClick={handleConditionOrgChange}>
                          {element}
                      </button>
                    )
                  })
        ...
        <div>
          <div className="condition-list-div">
            <button onClick={grouplistStateChange} className="icon-button">
              {!isGroupListVisible &&(<div><KeyboardArrowRightIcon/></div>)}
              {isGroupListVisible &&(<div><KeyboardArrowDownIcon/></div>)}
            </button>
            <div className="package-condition-title">
              Groups
            </div>
          </div>
          <div>
                  ...
                  grouplist.map((element, index)=>{
                    return(
                      <button
                        ...
                        value={element}
                        key={index}
                        onClick={handleConditionGroupChange}>
                          {element}
                      </button>
                    )
                  })
        ...
        <div>
          <div className="condition-list-div">
            <button onClick={taglistStateChange} className="icon-button">
              {!isTagListVisible &&(<div><KeyboardArrowRightIcon/></div>)}
              {isTagListVisible &&(<div><KeyboardArrowDownIcon/></div>)}
            </button>
            <div className="package-condition-title">
              Tags
            </div>
                  ...
                  taglist.map((element, index)=>{
                    return(
                      <button
                        ...
                        value={element}
                        key={index}
                        onClick={handleConditionTagChange}>
                          {element}
                      </button>
                    )
                  })
      ...
      <div className="datasetlist-conright-container">
        ...
        <div className="pageAmount">
          <font className="data-amount-div">{pageNow}&nbsp;/共{pagestotal}頁&nbsp;</font>
        </div>
      </div>
        <div className="icon-button-container">
