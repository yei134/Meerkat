...
const ConLeft=({owner_org, datasetTitle, groups})=>{
  var [orgInfo, setOrgInfo ] = useState();
  let datasetName = datasetTitle;
      ...
      try {
        await axios.get(
          `${process.env.REACT_APP_BACKEND_URI}api/ckan/organization_info`,
              {params:{id:owner_org},headers:{'Authorization': process.env.REACT_APP_CKAN_TOKEN}}
        )
        .then(response => {
